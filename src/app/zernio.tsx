...
export async function createZernioPresign({
  ...
  const response = await fetch(`${ZERNIO_API_BASE}/media/presign`, {
    method: "POST",
    headers: {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Zernio presign failed: ${response.status} ${body}`);
  }
  const data = await response.json();
  if (!data.uploadUrl || !data.publicUrl) {
    throw new Error("Zernio presign response missing uploadUrl/publicUrl");
  }
...
export async function createZernioDraft({
  ...
  if (!response.ok) {
    const body = await response.text().catch(() => "");
    throw new Error(`Zernio draft failed: ${response.status} ${body}`);
  }
}
